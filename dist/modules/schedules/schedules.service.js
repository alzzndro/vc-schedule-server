...
export class SchedulesService {
    ...
    static async getSchedulesByRoomId(roomId) {
        // Verify room exists
    ...
    static async createSchedule(data) {
        ...
        const conflicts = await SchedulesRepository.findOverlapping(room_id, day_of_week, start_time, end_time);
        if (conflicts.length > 0) {
            const conflict = conflicts[0];
            throw new ApiError(`Schedule conflict detected in ${room.name} on day ${day_of_week}: already booked for '${conflict.subject}' (${conflict.start_time} - ${conflict.end_time}) by teacher '${conflict.teacher}'`, 400);
        }
    ...
    static async updateSchedule(id, data) {
        ...
        const conflicts = await SchedulesRepository.findOverlapping(finalRoomId, finalDayOfWeek, finalStartTime, finalEndTime, id);
        if (conflicts.length > 0) {
            const conflict = conflicts[0];
            throw new ApiError(`Schedule conflict detected in ${roomName} on day ${finalDayOfWeek}: already booked for '${conflict.subject}' (${conflict.start_time} - ${conflict.end_time}) by teacher '${conflict.teacher}'`, 400);
        }
