...
export class RoomsRepository {
  ...
  static async findById(id: string): Promise<Room | null> {
    const result = await pool.query(
      `SELECT id, name, capacity, type, created_at, updated_at FROM rooms WHERE id = $1`,
      [id]
    );
    return result.rows[0] || null;
  }

  static async findByName(name: string): Promise<Room | null> {
    const result = await pool.query(
      `SELECT id, name, capacity, type, created_at, updated_at FROM rooms WHERE name = $1`,
      [name]
    );
    return result.rows[0] || null;
  }

  static async create(data: CreateRoomInput): Promise<Room> {
    const { name, capacity = 30, type = "Regular" } = data;
    const result = await pool.query(
      `INSERT INTO rooms (name, capacity, type, updated_at)
       VALUES ($1, $2, $3, NOW())
  ...
  static async update(id: string, data: UpdateRoomInput): Promise<Room | null> {
    const fields: string[] = [];
    const values: any[] = [];
    let idx = 1;
